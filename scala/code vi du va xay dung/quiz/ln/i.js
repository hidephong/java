...
var bcx_init = function(){
	...
	bouncex.cookie_domain = false;
				if(bouncex.calling_url.indexOf('Udemy.com')>0){
				bouncex.cookie_domain = 'Udemy.com';
			}
	...
	bouncex.wndsize = function(){
		...
		var init_vars = function() {
			...
			bouncex.varp = {max:0,c:0,timer:null,vn:[],suc:{},def:{}};
					...
					var n = 'logged_in';
					bouncex.vard[n] = { d: maked('false'), q: makeq('jQuery(\'a[href*=\"logout\"]\').length>0?true:false'),m:'none',p:false };
					initv(n);
					
					var n = 'ever_logged_in';
					bouncex.vard[n] = { d: maked('false'), q: makeq('bouncex.vars.logged_in || bouncex.vars.ever_logged_in'),m:'none',p:true };
					initv(n);
					
					var n = 'language';
					bouncex.vard[n] = { d: maked('0'), q: makeq('navigator.language'),m:'none',p:false };
					initv(n);
					
					var n = 'em_conv_bool';
					bouncex.vard[n] = { d: maked('0'), q: makeq('null'),m:'none',p:true };
					initv(n);
		...
		bouncex.make_include_string = function(type){
			...
			var bcx_str = window.location.protocol + '//bounceexchange.com/bounce/'+type+'.js?tojQ='+tojQ+'&tzo='+tzo+'&is_preview=false&website_id=858&resolution='+resolution+'&referrer='+bouncex.referrer+'&calling_url='+bouncex.calling_url+'&visit_cookie='+escape(visit_cookie)+'&cookie='+escape(cookie)+bcx_vars.replace(/\+/g,'%2B')+bcx_persist;
			return bcx_str;
		}
...
function setBounceVisitCookie(){
	var c_name = 'bounceClientVisit858v';
...
function getBounceVisitCookie(name){
	...
	function setBounceCookie(visit){
		var c_name = 'bounceClientVisit858';
					var exdate = new Date();
