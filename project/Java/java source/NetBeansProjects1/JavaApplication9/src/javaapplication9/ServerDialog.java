 ...
 * @author Duy Hoang
 */
public class ServerDialog extends javax.swing.JFrame {
...
int i=0;
   ...
   while(true){  i++;
                ...
                os[i] = new ObjectOutputStream(socket.getOutputStream());
                is[i] = new ObjectInputStream(socket.getInputStream());
                

                os[i].writeObject("Hello, connected");
                os[i].flush();

                  do
                {
                    try {
                        String strRead = is[i].readObject().toString();
    ...
    private void butSendActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_butSendActionPerformed
        ...
        try {
             os[i].writeObject(txtMess.getText());
             os[i].flush();
        } catch (Exception e) {
        }
